...
- `DESeq2_Shank3vsWT_allGenes.csv` — every gene tested, sorted by adjusted p-value
- `DESeq2_Shank3vsWT_sigDEGs.csv` — the differentially expressed genes
- `normCounts_Shank3vsWT.csv` — size-factor-normalized counts

`outputs/figures/` — open these from the file explorer

- `PCA_Shank3vsWT.png` — samples in PC space, coloured by genotype
- `SampleDistances_Shank3vsWT.png` — sample-to-sample distance heatmap
- `MAplot_Shank3vsWT.png`, `Volcano_Shank3vsWT.png` — the results

This is the same `outputs/figures/` + `outputs/tables/` split Hour 1 uses, so
you can compare the two runs file for file.

Each filename also ends in a **Run ID** — a timestamp like
`PCA_Shank3vsWT_20260730-144530.png`. Re-running never overwrites earlier
results, which is what lets you compare runs in Step 5.
...
```
Bootstrap: docker
From: bioconductor/bioconductor_docker:RELEASE_3_23-R-4.6.1   # the starting point
...
Right now a gene must clear **both** bars: statistically significant *and*
at least a two-fold change (`|log2FC| > 1`). Drop the effect-size bar and
keep every statistically significant gene:
...
The **Significant genes** count jumps from **3 to 16**. Same environment,
same data, same code — you asked a different question and got a different
answer. That's a controlled, reproducible experiment.

Your first run is still there. Every filename carries a **Run ID** timestamp,
so this run sat alongside it rather than overwriting it:

```bash
ls outputs/tables/
wc -l outputs/tables/DESeq2_Shank3vsWT_sigDEGs_*.csv
```

Two tables, both kept — 4 and 17 lines, which is a header row plus 3 genes
and a header row plus 16. Being able to put two runs next
to each other is what turns a change into *evidence* rather than just a new
number.

> **📝 What you just changed**
>
> `PADJ` is about **statistical** significance — how confident we are the
> difference is real. `LFC` is about **biological** significance — how large
> the difference is. Setting `LFC <- 0` keeps genes that are reliably
> different but only slightly so. Neither bar is the "right" one; which you
> use depends on the question you're asking.

Now that the effect-size filter is off, `PADJ` has room to bite too — try
`PADJ <- 0.01` and re-run for **16 → 9**.
