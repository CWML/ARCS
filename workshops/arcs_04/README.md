...
| **Async follow-along** | The rendered Quarto pages ([07](07-deseq2.qmd), [08](08-apptainer.qmd)) mirror the live session |
...
- **Modify** a provided Apptainer definition and re-run the pipeline from inside the container to confirm reproducible results *(Create)*
...
| 1:05–1:20 | Launch Codespace; confirm Apptainer + pre-built `.sif` are present; clone the Hour 1 repo | Codespaces |
| 1:20–1:50 | **Hour 2 — Apptainer.** Re-run the same pipeline via `apptainer exec`; modify the `.def` file; re-run | Codespaces |
...
arcs_04/
├── README.md            # this file
├── 07-deseq2.qmd        # Hour 1 lesson page (Positron + R Project)
├── 08-apptainer.qmd     # Hour 2 lesson page (Codespaces + Apptainer)
└── materials/
    ├── .devcontainer/
    │   ├── devcontainer.json   # Codespace config (Ubuntu 22.04, 4 CPU / 8 GB)
    │   └── setup.sh            # installs Apptainer + pulls pre-built .sif
    └── container/
        └── deseq2.def          # Apptainer definition built on Bioconductor
```

The pre-built DESeq2 image is published to a container registry and pulled
automatically when a learner opens the workshop repo in Codespaces — they do
not build the image during class. The `deseq2.def` file is what the *"Modify
and re-run"* exercise in Hour 2 edits.
...
- **"Modify," not "construct."** The Bloom's-level "Create" objective is hit by *editing a working definition* — adding a package, bumping a version — not by authoring a `.def` from scratch. This is achievable in 30 minutes; a from-scratch definition is not.
